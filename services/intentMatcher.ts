...
export class IntentMatcher {
    ...
    static match(questionId: string | number, transcript: string): any {
        ...
        if (questionId === 1) { // Contractions
            if (normalized.includes("بينهم دقيقة") || normalized.includes("دقيقتين") || normalized.includes("1-2")) return 2;
        ...
        const isYes = normalized.includes("نعم") || normalized.includes("ايوه") || normalized.includes("اه") || normalized.includes("بدي") || normalized.includes("حاسة") || normalized.includes("انفجر") || normalized.includes("تم") || normalized.includes("جاهز");
        const isNo = normalized.includes("لا") || normalized.includes("لأ") || normalized.includes("ما في") || normalized.includes("خفيف");

        if (questionId === 2 || questionId === 3 || questionId === 'baby_breathing' || questionId === 'placenta' || questionId === 'crowning' || questionId === 'universal_yes') {
