 ...
 * UPDATED WITH RANDOMIZED DELAYS.
 */
...
class DemoVoiceService {
    ...
    async listenForAnswer(questionId: string | number): Promise<SimulatedResult | null> {
        ...
        try {
            ...
            // 2. Simulate randomized listening duration (1.0 - 3.0s)
            const randomDelay = Math.floor(Math.random() * 2000) + 1000;
            await new Promise(resolve => setTimeout(resolve, randomDelay));

            // 3. Select a simulated response
            let sourceId = questionId === 'universal_yes' ? 'yes' : questionId;
            const possibleResponses = SIMULATED_VOICE_RESPONSES[sourceId as string | number];
